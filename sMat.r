...
##   - updated updateMat to return old matrix added to new MDS 9Sep04
##  Functions:
##
...
sMat <- function(z=NULL,xlim=c(-180,180),ylim=c(-90,90),dimXY=dim(z),
                 array=FALSE,...)
  ## sMat creates a "spatial matrix", as a list "x","y","z"
  ## - it is supported by overloaded functions, e.g. image.sMat
  ## z - the data matrix (can be empty or NULL)
  ## xlim/ylim - the spatial bounds 2-element vector
  ## dimXY - the size (in pixels) 2-element vector
  ## array - boolean dealing with mcmc simulation masks . . . 
{
  if (missing(z)) {
    if (is.null(dimXY)) {
      dimXY <- c(36,18);
...
updateMat <- function(mat,wps) {
  ## updateMat is a fast way of creating an image from
  ## a dataframe of lon/lat coordinates - each point contributes once to
  ## the pixel value
  ## mat - the (empty) sMat list
  ## wps - the coordinates in a dataframe (world coordinates e.g. lon/lat)
  ...
  mat2 <- sMat(t(mps)+mat$z,xlim=c(xmin,xmax),ylim=c(ymin,ymax))
}
image.sMat <- function(x,k=NULL,norescale=FALSE,grid=FALSE,...)
  ## image.sMat is the overloaded image function for sMat
  ##   it displays the matrix in a spatial context
  ##   resizing if appropriate etc.
  ##  x - the sMat list
  ##  k - mcmc mask stuff
  ##  norescale - should resizing be prevented?
  ##  grid - draw a grid of the "pixels"
  
  ## this 
{ ## catches matrices that are too big
  ...
  if (is.array(x$z) && !is.matrix(x$z)){
    ...
    x <- sMat(get.mask(x,k),xlim=range(x$x),ylim=range(x$y));
  }
...
contour.sMat <- function(x,k=1,norescale=FALSE,...) {
   ## contour.sMat is the overloaded image function for sMat
  ##   it adds a contour plot to the current display
  ##   resizing if appropriate etc.
...
rebinMat <- function(x,subs=4) {
  ## rebinMat resizes an sMat object using resampling
  ## i.e. every subs pixel is used
  ## if subs < 1 then the image will be made bigger!
...
increMat<- function(x,xy,incr=1,quiet=FALSE,debug=FALSE) {
  ## increMat increments the pixel value of an sMat matrix
  ## using coordinate (yx, c(lat,lon)),
  ## add one to cell containing point
  ## x - sMat list
  ## xy - coordinate vector c(x,y)
  ## incr - how much to add to the pixel that xy lies in?
  ## quiet - don't talk when the point is outside matrix bounds
  ...
  }else if (!quiet) cat(xy," is outside!\n")
  x
}
queryMat<- function(x,xy,incr=1,quiet=FALSE) {
  ## queryMat is the inverse of increMat
...
region <- function(x,xlim=c(-180,180),ylim=c(-90,90)) {
  ## region returns an AOI from an sMat, based on coordinate limits
  ## x - the sMat list
  ## xlim/ylim - bounds of AOI
  xs <- x$x[-length(x$x)]
...
# remember to reverse y indices for image vs. coordinate origins
    ...
    bits(masks$z[indX,indY,(k%/%31)+1],(k-1)%%31)
  } else {0}
}
